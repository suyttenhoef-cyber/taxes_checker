...
function parseRuns(line) {
  ...
  while ((m = rx.exec(line)) !== null) {
    if (m.index > pos) runs.push(new TextRun({ text: line.slice(pos, m.index) }))
    runs.push(new TextRun({ text: m[1], bold: true }))
    pos = m.index + m[0].length
  }
  if (pos < line.length) runs.push(new TextRun({ text: line.slice(pos) }))
...
export async function exportDocx({ texteGenere, params, profil }) {
  ...
  if (logo) {
    try {
      const imgData = b64ToUint8(logo)
      children.push(new Paragraph({
        children: [new ImageRun({ data: imgData, transformation: { width: 110, height: 65 }, type: 'png' })],
        spacing: { after: 100 },
      }))
    } catch (e) {
      console.warn('Logo ignoré:', e)
    }
  }

  // En-tête : table 2 colonnes (35% | 65%)
    ...
    rows: [new TableRow({
      children: [
        new TableCell({
          ...
          children: [
            new Paragraph({ children: [new TextRun({ text: 'Province de', size: 20 })] }),
            new Paragraph({ children: [new TextRun({ text: prov, bold: true, size: 22 })] }),
            new Paragraph({ text: '' }),
            new Paragraph({ children: [new TextRun({ text: 'Arrondissement de', size: 20 })] }),
            new Paragraph({ children: [new TextRun({ text: arr, bold: true, size: 22 })] }),
            new Paragraph({ text: '' }),
            new Paragraph({ text: '' }),
            new Paragraph({ children: [new TextRun({ text: `${typeCommune} de`, size: 20 })] }),
            new Paragraph({ children: [new TextRun({ text: commune.toUpperCase(), bold: true, size: 26 })] }),
          ],
        }),
        new TableCell({
          ...
          borders: CELL_BORDERS,
          children: [
            new Paragraph({ children: [new TextRun({ text: 'Du registre aux délibérations du Conseil Communal', size: 20 })] }),
            new Paragraph({ children: [new TextRun({ text: 'DE CETTE COMMUNE, A ETE EXTRAIT CE QUI SUIT :', bold: true, size: 20 })] }),
            new Paragraph({ text: '' }),
            new Paragraph({
              children: [new TextRun({ text: `SÉANCE DU ${dateSeance}`, bold: true, size: 22 })],
              alignment: AlignmentType.CENTER,
            }),
          ],
        }),
