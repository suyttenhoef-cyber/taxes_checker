...
export default function SignaturePanel({ docxBlob, params, profil, onSigne }) {
    ...
    <Panel>
      ...
      <ul className="list-none p-0 m-0 mb-4 text-left inline-block">
        {signers.length > 0
          ...
          : (
            <>
              <li className="py-1.5 text-[13px] text-vb-bleu">sebastien.uyttenhoef@vandenbroele.be — ⏳ En attente</li>
              <li className="py-1.5 text-[13px] text-vb-bleu">jullie.lesuisse@vandenbroele.be — ⏳ En attente</li>
            </>
          )
        }
      </ul>
  ...
  if (etape === 'signe') return (
    <Panel>
      ...
      <p className="text-vb-bleu text-[13px] mb-4">Le document a été signé par les deux signataires.</p>
      {signedBlob && (
